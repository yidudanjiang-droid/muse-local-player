...
> **以 APK 内置专题音频为核心、设备本地音乐为辅助的 Android 音乐播放器。v1.9.0 起，一个 APK 可同时承载多个独立专题；每期拥有自己的节目单、封面、章节、完成进度与断点，并通过 Material 3 专题目录切换。**
...
| 模块 | 当前能力 |
|---|---|
| 专题音频包 | 支持 legacy 单专题和 `featured_audio/topics/<专题目录>/` 多专题目录；每期可配置自己的元数据、封面、LRC、章节与笔记。首页专题目录与专题全览均可切换期数，并按稳定专题顺序组织完整连续队列。 |
...
| 专题播放旅程 | 首页以原创深靛蓝“收听路径”视觉呈现当前专题的完成计数、真实进度与下一步动作；每个专题的完成、断点与当前浏览选择均按稳定专题 ID 私有保存。专题全览统一呈现身份、版次、总时长、章节总数、完整节目单、完成状态、章节入口与节目笔记。 |
...
播放页使用 Muse 原创动态声场：播放时，静态 WebP 原画在端侧以低频漂移与呼吸光晕获得微妙运动；暂停时，背景固定并降低亮度。该效果不播放视频、不访问网络，也不会添加额外媒体解码器。曲名、进度与控制区始终位于半透明阅读层上；播放 APK 内专题时，曲目信息下会额外显示当前专题名称、完成进度与“节目单”入口，可直接回到专题全览选择曲目或章节。通过搜索、书签或节目单进入其他专题时，Muse 会自动切换到所属专题并建立该期完整连续队列，避免多个专题串队。未来若用户在本地添加 LRC 等歌词来源，可复用这块声场舞台呈现歌词，而不会捏造或联网抓取歌词内容。对于在 `pack.json` 中配置章节的专题，播放页的“章节进度”除显示当前章节、下一章前瞻与本章进度外，还提供**上一章、重播本章、下一章**的快捷定位；进度条下还会依照 Media3 已解析的真实时长按比例绘制章节节点，已经过的节点会强调显示。按钮与节点共用发布者定义的章节时间戳；未知时长时不会显示节点或猜测跳转位置。播放页还会显示当前输出设备类型（手机扬声器、有线耳机、USB 或蓝牙音频）；当其他应用接管音频、或音频设备断开导致系统保护暂停时，Muse 会说明原因并给出“继续”操作。异常文件的“重试”动作与中断后的“继续”动作由独立路径处理，不会互相误触发。
