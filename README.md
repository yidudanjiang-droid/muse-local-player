...
如果已扫描的设备文件后来被删除、移动，或当前设备无法解码该格式，Muse 会提示原因并自动尝试继续下一首可用曲目；单个异常文件不会直接中断整段队列播放。在随机或循环策略中，本轮已经确认失败的文件会持续被跳过，避免成功换歌后又循环回同一坏文件；用户主动点按该曲目时仍可重新尝试。MediaStore 自动刷新或手动重扫后，现有队列会依据最新索引移除失效设备文件：仍有有效曲目时优先保持当前曲目并继续播放，全部失效时会清空播放器与恢复状态；清理后的队列会写回 DataStore，避免下次启动反复恢复不存在的文件。
