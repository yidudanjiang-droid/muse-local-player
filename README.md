...
播放页使用 Muse 原创动态声场：播放时，静态 WebP 原画在端侧以低频漂移与呼吸光晕获得微妙运动；暂停时，背景固定并降低亮度。该效果不播放视频、不访问网络，也不会添加额外媒体解码器。曲名、进度与控制区始终位于半透明阅读层上；未来若用户在本地添加 LRC 等歌词来源，可复用这块声场舞台呈现歌词，而不会捏造或联网抓取歌词内容。
