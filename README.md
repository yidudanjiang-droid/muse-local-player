...
**专题播放旅程**仅面向 APK 内置的专题资源。首页会以已完成曲目数与进度条显示本期状态，并给出专题顺序里的首个未完成曲目；点按“继续旅程”会从该曲开始且保留完整专题队列，全部完成时可以“重新开始”。专题目录中的已完成曲目同步显示文字与完成标识，方便回看时快速分辨待听内容。完成状态仅在 Media3 提供真实时长并到达结尾附近时才写入此 APK 私有 DataStore；设备音乐、点击次数、章节/歌词/书签和其他共存包均不参与或共享该状态。
