...
播放页使用 Muse 原创动态声场：播放时，静态 WebP 原画在端侧以低频漂移与呼吸光晕获得微妙运动；暂停时，背景固定并降低亮度。该效果不播放视频、不访问网络，也不会添加额外媒体解码器。曲名、进度与控制区始终位于半透明阅读层上；未来若用户在本地添加 LRC 等歌词来源，可复用这块声场舞台呈现歌词，而不会捏造或联网抓取歌词内容。播放页还会显示当前输出设备类型（手机扬声器、有线耳机、USB 或蓝牙音频）；当其他应用接管音频、或音频设备断开导致系统保护暂停时，Muse 会说明原因并给出“继续”操作。异常文件的“重试”动作与中断后的“继续”动作由独立路径处理，不会互相误触发。
