...
长队列会按 **队列前序、正在播放、接下来** 分段；打开队列时会自动定位到当前曲目区域，标题同时显示总曲数和当前播放序号。当前曲目前有已播歌曲时，队列顶部会出现 **移除已播**：它只清理队列前序，不会改变当前曲目或接下来播放顺序，并可即时撤销。首页专题主卡片现同时提供 **从头播放** 与 **加入队列**：前者建立完整专题队列，后者会去重后追加当前专题的新增曲目，避免打断正在收听的设备音乐或收藏内容。该机制参考商业播放器中成熟的队列整理模式，但已按 Muse 的离线本地播放架构原创实现。
