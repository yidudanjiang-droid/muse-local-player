...
| `pack.json` | JSON 合法，标题、导语、默认艺人和默认专辑符合本期主题；多专题发行时，每个 `featured_audio/topics/<专题目录>/pack.json` 的稳定 `id` 不重复且保持不变。 |
...
for apk in app/build/outputs/apk/release/*.apk; do
  ...
  $ANDROID_HOME/build-tools/<version>/aapt dump badging "$apk"
done
...
如果在构建完成后向 APK `assets/` 加入音频，必须在修改后重新对 APK 签名，并在设备上安装或覆盖更新该 APK。Muse 会在启动时发现 legacy `featured_audio/` 单专题结构，或 `featured_audio/topics/<专题目录>/` 的多专题目录；每期的 `pack.json` 独立控制文案、封面、章节与节目笔记。二次打包工具若压缩音频，应用会自动准备私有缓存副本以保证播放。

重新签名会改变 APK 签名证书。对于已有用户的正式更新，必须继续使用与已发布版本相同的签名密钥，否则 Android 会拒绝覆盖安装。[1]

## GitHub 发布建议

公开仓库只提交源代码、许可证、notice、模板和文档。不要提交 `.jks`、`keystore.properties`、APK、AAB、mapping 文件、`local.properties`、`.gradle` 或 `app/build`。可将经过核验的 `arm64-v8a`、`armeabi-v7a`、`x86_64`、`x86` 与 universal 五个 APK 及一份 SHA-256 清单上传至 GitHub Release；发布说明至少包含版本号、主要变化、签名方案、ABI 下载建议和版权提醒。
